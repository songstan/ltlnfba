...
{       FILE *inp, *out;
        ...
        out = fopen(tgt, "w");
        if (!inp || !out)
        {       printf("ltl_nf_ba: cannot cp %s to %s\n", src, tgt);
                alldone(1);
        }
...
usage(void)
{
        printf("usage: ltl_nf_ba [-flag] -f formula\n");
        printf("                   or -F file\n");
        ...
        printf(" -n\t\tdisable to construct the buchi automata and  construct the GBA only\n");
        printf(" -p\t\table to print the GBA and BA\n");
...
{	int i;
	...
	while (argc > 1 && argv[1][0] == '-')
        {       switch (argv[1][1]) {
                ...
                case 'f': add_ltl = (char **) argv;
                          argc--; argv++; break;
                case 'n': tl_dis_ba = 0; break;
                case 'p': tl_print_ba = 1; break;
	...
	if(!ltl_file && !add_ltl) usage();
        ...
        {       char formula[4096];
                ...
                {       printf("ltl_nf_ba: cannot open %s\n", *ltl_file);
                        alldone(1);
                }
        ...
        {       char cmd[128], out2[64];
                strcpy(out1, "_tmp1_");
                strcpy(out2, "_tmp2_");
                /*printf("the LTL formula string is %s\n",argv[1]);*/
